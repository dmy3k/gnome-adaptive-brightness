export class BucketMapper {
   ...
   * Optimized: checks if both values are in the same bucket without expensive lookups
  ...
  crossesBucketBoundary(previousLux, currentLux) {
    if (previousLux === null || currentLux === null) {
      return true;
    }

    for (const bucket of this.buckets) {
      const prevInBucket = this.isLuxInBucket(previousLux, bucket);
      const currInBucket = this.isLuxInBucket(currentLux, bucket);

      if (prevInBucket && currInBucket) {
        return false; // Both in same bucket, no change
      }
      if (prevInBucket || currInBucket) {
        return true; // One in, one out = boundary crossed
      }
    }
