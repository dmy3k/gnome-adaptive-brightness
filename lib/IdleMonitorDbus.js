...
export class IdleMonitorDbus {
  constructor() {
    ...
    // Bumped by stopMonitoring() to invalidate any AddIdleWatch/AddUserActiveWatch
    // call already in flight, so its result can't resurrect a watch ID after
    // monitoring was told to stop.
    this._generation = 0;
  }
    ...
    const watchId = await new Promise((resolve, reject) => {
      this._proxy.AddIdleWatchAsync(this._timeoutMs, (result, error) => {
        if (error) {
          ...
          resolve(typeof result === 'number' ? result : Number(result));
        }
      });
    });

    // stopMonitoring()/destroy() ran while this call was in flight - don't
    // resurrect a watch that was just told to go away.
    if (this._destroyed || generation !== this._generation) {
      this._removeWatchById(watchId).catch(() => {});
      return;
    }
