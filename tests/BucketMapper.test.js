...
describe("BucketMapper", () => {
  ...
  beforeEach(() => {
    ...
    testBuckets = [
      ...
      { min: 5, max: 200, brightness: 25 },
      { min: 50, max: 650, brightness: 50 },
      { min: 350, max: 2000, brightness: 75 },
      { min: 1000, max: 10000, brightness: 100 },
    ];
  ...
  describe("mapLuxToBrightness", () => {
    test("should return correct brightness for lux value", () => {
      ...
      expect(result).toEqual({ min: 5, max: 200, brightness: 25 });
    });
    ...
    test("should demonstrate hysteresis behavior preventing rapid bucket switching", () => {
      ...
      expect(mapper.currentBucketIndex).toBe(2);
      expect(mapper.buckets[2]).toEqual({ min: 50, max: 650, brightness: 50 });
      ...
      expect(mapper.currentBucketIndex).toBe(1);
      expect(result2.brightness).toBe(25);
    });

    test("should work with single bucket", () => {
      const singleBucket = [{ min: 0, max: 1000, brightness: 50 }];
  ...
  describe("integration scenarios", () => {
    test("should handle a typical day cycle of light changes", () => {
      ...
      expect([25, 50]).toContain(result.brightness);
